...
STAGE_PROGRESS = {"crawl": 0.15, "recon": 0.35, "detect": 0.55, "triage": 0.72, "analyze": 0.87, "report": 1.0}
...
def pipeline_html(current_stage):
    ...
    for i, (key, label, icon) in enumerate(PIPELINE_STAGES):
        ...
        connector = f"""
        <div style="width:40px;height:2px;background:#334155;"></div>
        ...
        #             {icon_display}
        #         </div>
        #         <span style="font-size:11px;color:{label_color};font-weight:600;margin-top:6px;text-align:center;">
        #             {label}
        #         </span>
        #     </div>
        #     {connector}
...
def render():
    ...
    #     st.markdown(pipeline_html("crawl"), unsafe_allow_html=True)
    ...
    current_stage = ["crawl"]
    stage_map = {"crawl": "crawl", "recon": "recon", "detect": "detect", "triage": "triage", "analyze":"analyze","report": "report"}
    ...
    spinner_placeholder.markdown(f"""
        ...
        <div style="color:#94a3b8;font-size:16px;">Scan in progress...</div>
        <div style="color:#06b6d4;font-size:13px;margin-top:6px;">
        Current stage: {current_stage[0]}
        </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    def progress_callback(stage, message, data=None):
        ...
        logs.append((stage.upper(), message))       
        log_area.markdown(
            "<div style='background:#0d1117;border:1px solid #1e293b;border-radius:10px;"
            "padding:14px 18px;font-family:monospace;font-size:12px;max-height:200px;overflow:auto;'>"
            # + "<br>".join(
            #     f"<span style='color:#06b6d4;'>[{l.split(']')[0].replace('**[', '')}]</span>"
            #     f"{l.split(']', 1)[-1]}"
            #     for l in logs[-15:]
            + "<br>".join(
                f"<span style='color:#06b6d4;'>[{stage}]</span> {msg}"
                for stage, msg in logs[-15:]
            )
            + "</div>",
            unsafe_allow_html=True,
        )
        if stage in stage_map:
            ...
            # pipeline_container.markdown(pipeline_html(current_stage[0]), unsafe_allow_html=True)
            pipeline_placeholder.markdown(
                pipeline_html(current_stage[0]),
                unsafe_allow_html=True
            )
